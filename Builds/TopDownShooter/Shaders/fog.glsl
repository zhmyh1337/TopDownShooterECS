...
uniform mat4 viewProjectionInversed;

void main() 
{
    gl_Position = vec4(Position, 1, 1);
    world = vec2(viewProjectionInversed * gl_Position);
}
...
void main()
{
    ...
    float frequency = 0.4;
    const float frequencyRatio = 2;
    float significance = 0.25;
    const float significanceRatio = 0.5;
    float noise = 0;
    for (int i = 0; i < octavesCount; ++i)
    {
        noise += significance * (perlinNoise(vec3(world, time * 0.5) * frequency));

        frequency *= frequencyRatio;
        significance *= significanceRatio;
    }
